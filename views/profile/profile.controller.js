...
$('.avatar-view').css('height',($(window).width() * 250) / 1024);*/
    	  $(function () {
    		  $timeout(function(){$('.list-group.table-condensed').show()},500);
    	  });
    	...
    	$scope.gotoAnchor = function(x) {
    		...
    		$rootScope.loadUserPage = function(){
    			...
    			var fullurl = 'service/sirfUser/getuserPage?userID='+$routeParams.userId+'&limit=25&offset=0&currentUser='+$rootScope.userDetails.user_Id;
    		...
    		$scope.subscribeuser = function(user){
    			var fullurl = 'service/sirfUser/subscriberPage';
    			  $http({
    					method: 'POST',
    					url: fullurl,
    					data: {"subscriberUserId":user, "action":"ADD"},//'Action=UP_VOTE&topicId='+$scope.currentID
    				}).then(function (data){
